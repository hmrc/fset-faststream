...
trait ExclusiveScheduledJob extends ScheduledJob {
  ...
  final def execute(implicit ec: ExecutionContext): Future[Result] = {
    if (mutex.tryAcquire()) {
      ...
      Future.successful(Result("Skipping execution: another job is running"))
    }
  }

  def isRunning: Future[Boolean] = Future.successful(mutex.availablePermits() == 0)

  // Use a binary semaphore as a mutual exclusion object to synchronize which job can run
  // Note: binary semaphores are non-reentrant by nature, implying that the same thread can't re-acquire a critical section
