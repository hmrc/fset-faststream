...
trait OnlineTestService extends ResetPhase1Test {
  ...
  def resetPhase1Tests(application: OnlineTestApplication, testNamesToRemove: List[String]): Future[Events] = {
    for {
    - <- registerAndInviteForTestGroup(application, testNamesToRemove)
      ...
      MongoEvents.OnlineExerciseReset(application.applicationId) ::
      Nil
    }
  }
