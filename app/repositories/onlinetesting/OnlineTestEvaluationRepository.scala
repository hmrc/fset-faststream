  ...
  val nextApplicationQuery = (currentPassmarkVersion: String) =>
    Document("$and" -> BsonArray(
      ...
      Document("$or" -> BsonArray(
        Document(s"testGroups.$phase.evaluation.passmarkVersion" -> Document("$exists" -> false)),
        Document(s"testGroups.$phase.evaluation.passmarkVersion" -> Document("$ne" -> currentPassmarkVersion)),
        Document("$expr" -> Document.apply(" { $ne: [ \"$testGroups.PHASE2.evaluation.previousPhasePassMarkVersion\", \"$testGroups.PHASE1.evaluation.passmarkVersion\"] } "))
      ))
    ))
  //scalastyle:on
}
...
class Phase3EvaluationMongoRepository @Inject() (appConfig: MicroserviceAppConfig,
  ...
  val nextApplicationQuery = (currentPassmarkVersion: String) => {
    ...
    val passMarksHaveChanged = Document("$or" -> BsonArray(
      Document("$expr" -> Document.apply(" { $ne: [ \"$testGroups.PHASE3.evaluation.previousPhasePassMarkVersion\", \"$testGroups.PHASE2.evaluation.passmarkVersion\"] } ")),
      Document("$expr" -> Document.apply(" { $ne: [ \"$testGroups.PHASE3.evaluation.previousPhaseResultVersion\", \"$testGroups.PHASE2.evaluation.resultVersion\"] } "))
    ))
    //scalastyle:on
