    ...
    val latestAVTRCallbackOpt = activeVideoTestReviewedCallbacksOpt.flatMap {
      reviewedCallbacks =>
        reviewedCallbacks.sortWith { (r1, r2) =>
          val r1Received = Codecs.fromBson[DateTime](r1.get("received").asDateTime())
          val r2Received = Codecs.fromBson[DateTime](r2.get("received").asDateTime())
  ...
  private def onlineTests(doc: Document): Map[String, List[Option[String]]] = {
    ...
    def extractDataFromTest(test: Option[BsonDocument]) = {
      ...
      List(
        ...
        test.map( _.get("invitationDate").asDateTime().getValue.toString ),
        test.map( _.get("startedDateTime").asDateTime().getValue.toString ),
        test.map( _.get("completedDateTime").asDateTime().getValue.toString ),
