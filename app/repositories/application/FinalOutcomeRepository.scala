...
import model.persisted.{ContactDetailsUserId, FsbTestGroup}
...
class FinaOutcomeMongoRepository @Inject() (val dateTimeFactory: DateTimeFactory,
  ...
  override def nextApplicationForFinalFailureNotification(batchSize: Int): Future[Seq[ApplicationForProgression]] = {
    ...
    applicationForProgressionCollection.find(query).limit(batchSize).toFuture()
//    selectRandom[BSONDocument](query, batchSize).map(_.map(doc => doc: ApplicationForProgression)) //TODO: mongo fix this
  }
