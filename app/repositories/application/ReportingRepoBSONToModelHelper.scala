...
object ReportingRepoBSONToModelHelper extends ReportingRepoBSONToModelHelper

trait ReportingRepoBSONToModelHelper extends BaseBSONToModelHelper {

  def toReportWithPersonalDetails(doc: BSONDocument): ReportWithPersonalDetails = {
    ...
    val progress: ProgressResponse = toProgressResponse(doc, applicationId)
  ...
  def toCandidateProgressReportItem(doc: BSONDocument): CandidateProgressReportItem = {
  ...
  def toApplicationForDiversityReport(doc: BSONDocument): ApplicationForDiversityReport = {
    val schemesDoc = doc.getAs[BSONDocument]("scheme-preferences")
    val schemes = schemesDoc.flatMap(_.getAs[List[SchemeType]]("schemes"))
  ...
  def toApplicationForOnlineTestPassMarkReport(doc: BSONDocument): ApplicationForOnlineTestPassMarkReport = {
