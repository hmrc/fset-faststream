...
object FSACIndicatorCSVRepository extends FSACIndicatorCSVRepository {
  ...
  override def find(postcode: Option[String], outsideUk: Boolean): Option[String] = {
  ...
  override def findForCandidateProgressReport(postcode: Option[String], candidate: CandidateProgressReportItem): Option[String] = {
  ...
  private def find(postcode: Option[String]): Option[String] = {
    postcode.flatMap(postCodeVal => {
      val key = postCodeVal.takeWhile(!_.isDigit).toUpperCase
      indicators.get(key).fold[Option[String]](Some(DefaultIndicator))(indicator => Some(indicator.assessmentCentre))
    })
  }
...
trait FSACIndicatorCSVRepository extends CsvHelper {
  ...
  def find(postcode: Option[String], outsideUk: Boolean): Option[String]
  def findForCandidateProgressReport(postcode: Option[String], candidate: CandidateProgressReportItem): Option[String]
}
