...
trait QuestionnaireRepository {
  ...
  def findForOnlineTestPassMarkReport(userIds: List[String]): Future[Map[String, QuestionnaireReportItem]]
...
class QuestionnaireMongoRepository(socioEconomicCalculator: SocioEconomicScoreCalculator)(implicit mongo: () => DB)
  ...
  override def findForOnlineTestPassMarkReport(userIds: List[String]): Future[Map[String, QuestionnaireReportItem]] = {
    ...
    val query =
      ...
      BSONDocument("userId" -> BSONDocument("$in" -> userIds))
