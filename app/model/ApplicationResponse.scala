...
//TODO: mongo temp version with no ProgressResponse. Remove once ProgressResponse is working
case class ApplicationResponse2(applicationId: String,
                               applicationStatus: String,
                               applicationRoute: ApplicationRoute,
                               userId: String,
                               testAccountId: String,
//                               progressResponse: ProgressResponse, //TODO: mongo fix
                               civilServiceExperienceDetails: Option[CivilServiceExperienceDetails],
                               overriddenSubmissionDeadline: Option[DateTime])

object ApplicationResponse2 {
  implicit val applicationResponseFormat: OFormat[ApplicationResponse2] = Json.format[ApplicationResponse2]

  val mongoFormat: Format[ApplicationResponse2] = (
    (__ \ "applicationId").format[String] and
      (__ \ "applicationStatus").format[String] and
      (__ \ "applicationRoute").format[ApplicationRoute] and
      (__ \ "userId").format[String] and
...
//      (__ \ "progressResponse").format[ProgressResponse] and
      (__ \ CivilServiceExperienceDetails.root).formatNullable[CivilServiceExperienceDetails] and
      (__ \ "overriddenSubmissionDeadline").formatNullable[DateTime]
    )(ApplicationResponse2.apply, unlift(ApplicationResponse2.unapply))
}
