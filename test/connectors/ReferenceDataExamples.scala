...
object ReferenceDataExamples {
  ...
  object Schemes {
    ...
    val GovStats = Scheme("GovernmentStatisticalService", "GSS", "Government Statistical Service", Some(SiftRequirement.FORM),
      evaluationRequired = true)
    val HoP = Scheme("HousesOfParliament", "HoP", "Houses Of Parliament", Some(SiftRequirement.FORM), evaluationRequired = true)
    val HR = Scheme("HumanResources", "HRFS", "Human Resources", None, evaluationRequired = false)
    val ProjectDelivery = Scheme("ProjectDelivery", "PDFS", "Project Delivery", Some(SiftRequirement.FORM), evaluationRequired = true)
    val SciEng = Scheme("ScienceAndEngineering", "SEFS", "Science And Engineering", Some(SiftRequirement.FORM), evaluationRequired = true)
