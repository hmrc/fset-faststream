...
class EventsControllerSpec extends UnitWithAppSpec {
  ...
  trait TestFixture extends TestFixtureBase {
    ...
    val event = Event("id", EventType.FSAC, "description", location, venue,
            LocalDate.now, 32, 10, 5, LocalTime.now, LocalTime.now, OffsetDateTime.now, Map.empty, List.empty)
    val eventWithAllocationsSummaryWithDescription =
      EventWithAllocationsSummary(LocalDate.now, event, candidateAllocations = Nil, assessorAllocations = Nil)
  }
}
