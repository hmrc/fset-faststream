...
class DashboardPageSpec extends PlaySpec with TableDrivenPropertyChecks {
  ...
  val Applications = Table(
    ...
    (PHASE1_TESTS,                             ProgressActive,   ProgressActive,   ProgressInactiveDisabled,  ProgressInactiveDisabled,  true,                                   false,                                    false,                    false,                             false,                       true,                  true,   "John Biggs", None, None,  ASSESSMENT_STATUS_UNKNOWN,          POSTASSESSMENT_STATUS_UNKNOWN)
  ...
  "The Steps visibility" should {
    ...
    "be tested for all statuses" ignore {
      val statusesTested = Applications.toList.map { case (status, _, _, _, _t , _, _, _, _, _, _, _, _, _, _, _, _) => status }
