...
object EventExamples {
  ...
  val e1 = Event(id = "1", eventType = EventType.FSAC, description = "PDFS FSB", location = LocationLondon,
    venue = VenueLondon, date = LocalDate.now(), startTime = LocalTime.now(), endTime = LocalTime.now().plusHours(3), skillRequirements = Map(),
    sessions = List())

  val EventsNew = List(
    e1,
    Event(id = UUIDFactory.generateUUID(), eventType = EventType.FSAC, description = "GCFS FSB", location = LocationLondon,
      venue = VenueLondon, date = LocalDate.now(), startTime = LocalTime.now().plusMinutes(30), endTime = LocalTime.now().plusHours(3),
    ...
    Event(id = UUIDFactory.generateUUID(), eventType = EventType.SKYPE_INTERVIEW, description = "GCFS FSB", location = LocationNewcastle,
      venue = VenueNewcastle, date = LocalDate.now(), startTime = LocalTime.now(), endTime = LocalTime.now().plusHours(3),
    ...
    Event(id = UUIDFactory.generateUUID(), eventType = EventType.FSAC, description = "DFS FSB", location = LocationNewcastle,
      venue = VenueNewcastle, date = LocalDate.now(), startTime = LocalTime.now(), endTime = LocalTime.now().plusHours(3),
      skillRequirements = Map(SkillType.QUALITY_ASSURANCE_COORDINATOR.toString -> 1), sessions = List())
  ...
  val YamlEvents = List(
    Event("1", EventType.withName("FSAC"), "PDFS FSB", Location("London"), Venue("london fsac", "bush house"),
      LocalDate.parse("2017-04-03"), LocalTime.parse("11:00:00.000"), LocalTime.parse("12:00:00.000"),
    ...
    Event("2", EventType.withName("FSAC"), "PDFS FSB", Location("London"), Venue("london fsac", "bush house"),
      LocalDate.parse("2017-04-03"), LocalTime.parse("09:00:00.000"), LocalTime.parse("12:00:00.000"),
