...
class Phase1TestServiceSpec extends UnitSpec with ExtendedTimeout
  ...
  "register and invite application" should {
    ...
    "fail if registration fails" in new OnlineTest {
      ...
      verify(auditServiceMock, never()).logEventNoRequest(any[String], any[Map[String, String]])(any[ExecutionContext])
    }
  ...
  "mark as completed" should {
    ...
    "not update the completed timestamp or the completed progress status if they already exist" in new OnlineTest {
      private val myPhase1TestProfile = Phase1TestProfile(
        expirationDate,
        List(phase1Test.copy(orderId = orderId, startedDateTime = Some(startedDate), completedDateTime = Some(startedDate.plusDays(1))))
      )
      when(phase1TestRepositoryMock.getTestGroupByOrderId(anyString()))
        .thenReturnAsync(Phase1TestGroupWithUserIds("appId123", userId, myPhase1TestProfile))
      ...
      when(appRepositoryMock.getProgressStatusTimestamps(anyString()))
        .thenReturnAsync(
          List(
            "PHASE1_TESTS_STARTED" -> OffsetDateTime.now(),
            "PHASE1_TESTS_COMPLETED" -> OffsetDateTime.now()
          )
        )
      ...
      verify(phase1TestRepositoryMock, never()).updateProgressStatus("appId123", ProgressStatuses.PHASE1_TESTS_COMPLETED)
    }
  }
