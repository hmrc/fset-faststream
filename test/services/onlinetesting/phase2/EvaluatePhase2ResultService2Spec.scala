...
class EvaluatePhase2ResultService2Spec extends BaseServiceSpec {
  ...
  trait TestFixture {
    ...
    def createAppWithTestGroupDeleteme(tests: List[CubiksTest]) = {
    ...
    def createSdipFaststreamAppWithTestGroupDeleteMe(tests: List[CubiksTest]) = {
      val phase2 = Phase2TestProfileExamples.profile.copy(tests = tests)
