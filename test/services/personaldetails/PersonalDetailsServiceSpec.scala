...
class PersonalDetailsServiceSpec extends UnitWithAppSpec with ShortTimeout {
  ...
  "find candidate" should {
    "return personal and contact details" in {
      ...
      when(mockCivilServiceExperienceDetailsRepository.find(AppId)).thenReturn(Future.successful(Some(CivilServiceExperienceDetails(applicable = false))))
  ...
  "update candidate" should {
    "update personal and contact details" in {
      ...
      when(mockCivilServiceExperienceDetailsRepository.update(AppId, CandidateContactDetailsUK.civilServiceExperienceDetails.get)).thenReturn(emptyFuture)
      when(mockFSACIndicatorCSVRepository.find(any(), any())).thenReturn(Some(model.FSACIndicator("London", "London")))
      when(mockFSACIndicatorRepository.update(eqTo(AppId), eqTo(UserId), eqTo(FSACIndicator("London", "London", "1")))).thenReturn(emptyFuture)
