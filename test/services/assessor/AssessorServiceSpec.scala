...
class AssessorServiceSpec extends BaseServiceSpec {
  ...
  "updating an assessors allocation status" must {
    ...
    "notify assessors of new events" in new AssessorsEventsSummaryFixture {
      ...
      val fsacVirtual = s"${DateTimeFormatter.ofPattern("EEEE, dd MMMM YYYY").format(now)} (FSAC - Virtual)"
      val fsbLondon = s"${DateTimeFormatter.ofPattern("EEEE, dd MMMM YYYY").format(now)} (FSB - London)"
      val fsbNewcastle = s"${DateTimeFormatter.ofPattern("EEEE, dd MMMM YYYY").format(now)} (FSB - Newcastle)"
...
//    val mockemailClient = mock[CSREmailClient] //TODO:fix changed type
  ...
  trait AssessorsEventsSummaryFixture extends TestFixture {
    ...
    val availabilities = Set(AssessorAvailability(Location("london"), LocalDate.of(2017, 8, 11)))
