...
class EvaluatePhase1ResultJobSpec extends UnitWithAppSpec {
  ...
  "Scheduler execution" should {
    "evaluate applications ready for evaluation" in new TestFixture {
      when(mockEvaluateService2.nextCandidatesReadyForEvaluation(any[Int])(any[Format[Phase1PassMarkSettings]]))
    ...
    "do not evaluate candidate if none of them are ready to evaluate" in new TestFixture {
      ...
      verify(mockEvaluateService2, never).evaluate(any[ApplicationReadyForEvaluation2], any[Phase1PassMarkSettings])
    }
  }
  ...
  trait TestFixture {
    ...
    val apps = (1 to 10 map { id =>
      ApplicationReadyForEvaluation2(s"app$id", ApplicationStatus.PHASE1_TESTS, ApplicationRoute.Faststream, isGis = false,
        profile.activeTests, None, None, schemes)
    }) ++ (1 to 10 map { id =>
      ApplicationReadyForEvaluation2(s"edipAppId$id", ApplicationStatus.PHASE1_TESTS, ApplicationRoute.Edip, isGis = false,
        profile.activeTests, None, None, schemes)
    })
