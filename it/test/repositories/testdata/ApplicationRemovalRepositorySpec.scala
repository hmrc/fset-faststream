...
class ApplicationRemovalRepositorySpec extends MongoRepositorySpec {
  ...
  def insert(doc: Document): Future[InsertOneResult] = applicationCollection.insertOne(doc).toFuture()
  def findByAppStatus(applicationStatus: ApplicationStatus): Future[Seq[Document]] =
  ...
  "Application removal repository" should {
    "remove the expected applications" in {
      ...
      for {
        insertedDocs <- insertDocs
        _ = insertedDocs.size mustBe 2
        deletedUserIds <- repository.remove(Some(CREATED))
        docsAfterDeletion <- findByAppStatus(CREATED)
      } yield {
