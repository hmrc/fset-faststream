...
class CandidateAllocationRepositorySpec extends MongoRepositorySpec {
  ...
  val allocations: Seq[CandidateAllocation] = Seq(
    CandidateAllocation(appId1, "eventId1", "sessionId1", UNCONFIRMED, "version1", None, LocalDate.now(), reminderSent = false),
    CandidateAllocation(appId2, "eventId1", "sessionId1", CONFIRMED, "version1", None, LocalDate.now(), reminderSent = false),
    CandidateAllocation(appId3, "eventId2", "sessionId2",  UNCONFIRMED, "version1", None, LocalDate.now().minusDays(6), reminderSent = false)
  )
