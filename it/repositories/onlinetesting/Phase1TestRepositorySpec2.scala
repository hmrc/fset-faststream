...
// Tests PSI based code
class Phase1TestRepositorySpec2 extends MongoRepositorySpec with ApplicationDataFixture {
...
*/
  ...
  val TestProfile = Phase1TestProfile2(expirationDate = DatePlus7Days, tests = List(phase1Test))
  val testProfileWithAppId = Phase1TestGroupWithUserIds2(
    "appId",
    "userId",
  ...
  "Next phase1 test group with report ready" should {
    "not return a test group if the progress status is not appropriately set" in {
      createApplicationWithAllFields2("userId", "appId", "testAccountId", "frameworkId", "PHASE1_TESTS",
    ...
    "return a test group if only one report is ready to download" in {
      ...
      createApplicationWithAllFields2("userId2", "appId2", "testAccountId", "frameworkId", "PHASE1_TESTS",
        additionalProgressStatuses = List((PHASE1_TESTS_COMPLETED, true), (PHASE1_TESTS_RESULTS_READY, true)),
      ...
      val phase1TestResultsReady = phase1TestRepo.nextTestGroupWithReportReady.futureValue
      phase1TestResultsReady.isDefined mustBe true
      phase1TestResultsReady.get mustBe Phase1TestGroupWithUserIds2("appId2", "userId2", profile)
    }
  }
  ...
  "The OnlineTestApplication case model" should {
    "be correctly read from mongo" in {
      createApplicationWithAllFields2("userId", "appId", "testAccountId", "frameworkId", "SUBMITTED", isGis = true).futureValue
  ...
  "nextExpiringApplication" should {
    ...
    val testProfile = Phase1TestProfile2(expirationDate = date, tests = List(phase1Test))
    ...
    "return no results" when {
      ...
      "the date is not expired yet" in {
        ...
        phase1TestRepo.insertOrUpdateTestGroup(
          AppId,
          Phase1TestProfile2(expirationDate = new DateTime().plusHours(2), tests = List(phase1Test))).futureValue
        phase1TestRepo.nextExpiringApplication(phase1ExpirationEvent).futureValue mustBe None
      }
  ...
  "nextTestForReminder" should {
    ...
    "return no results" when {
      ...
      "the expiration date is in 26h but we send the second reminder only after 24h" in {
        ...
        phase1TestRepo.insertOrUpdateTestGroup(
          AppId,
          Phase1TestProfile2(expirationDate = new DateTime().plusHours(30), tests = List(phase1Test))).futureValue
        phase1TestRepo.nextTestForReminder(Phase1SecondReminder).futureValue mustBe None
      }
  ...
  "remove test group" should {
    ...
    "remove the test group" in {
      createApplicationWithAllFields2("userId", "appId", "testAccountId", appStatus = ApplicationStatus.PHASE1_TESTS).futureValue
      phase1TestRepo.insertOrUpdateTestGroup("appId", testProfile).futureValue
  ...
  "Progress status" should {
    ...
    "update progress status should not update if the Application Status is different from that which is being set" in {
      createApplicationWithAllFields2("userId", "appId", "testAccountId", appStatus = ApplicationStatus.PHASE2_TESTS).futureValue
    ...
    "reset progress statuses" in {
      createApplicationWithAllFields2("userId", "appId", "testAccountId", appStatus = ApplicationStatus.PHASE1_TESTS,
    ...
    "reset progress statuses when phase1 tests are failed" in {
      ...
      createApplicationWithAllFields2("userId", "appId", "testAccountId", appStatus = ApplicationStatus.PHASE1_TESTS_FAILED,
        additionalProgressStatuses = List(ProgressStatuses.PHASE1_TESTS_INVITED -> true,
          ProgressStatuses.PHASE1_TESTS_COMPLETED -> true,
          ProgressStatuses.PHASE1_TESTS_FAILED -> true)).futureValue
      phase1TestRepo.insertOrUpdateTestGroup("appId", Phase1TestProfile2(now, phase1TestsWithResult)).futureValue
  ...
  "update psi test" should {
    ...
    "add the completion time for a psi test if the test group has not expired" in {
      ...
      val input = Phase1TestProfile2(expirationDate = now.plusDays(5), tests = List(phase1Test))

      createApplicationWithAllFields2("userId", "appId", "testAccountId", "frameworkId", "PHASE1_TESTS", phase1TestProfile = Some(input))
        .futureValue
